    ...
    new (class OneBotv11Adapter {
        ...
        async recallMsg(data, message_id) {
            ...
            for (const i of message_id)
                msgs.push(
                    await data.bot.sendApi("delete_msg", {
                        message_id: i
                    })
                )
            return msgs
        ...
        async getMsg(data, message_id) {
            const msg = (
                await data.bot.sendApi("get_msg", {
                    message_id
                })
            ).data
        ...
        pickGroup(data, group_id) {
            ...
            return {
                ...
                get is_admin() {
                    return data.bot.gml.get(group_id)?.get(data.self_id)?.role === "admin"
                }
            }
        }
        ...
        async makeNotice(data) {
            switch (data.notice_type) {
                ...
                case "group_increase":
                    ...
                    if (data.user_id === data.self_id) data.bot.getGroupMemberMap()
                    else data.bot.pickGroup(data.group_id).getMemberMap()
                    break
                case "group_decrease":
                    Bot.makeLog(
                        "info",
            ...
            if (data.post_type) {
                ...
                if (
                    ![
                        0,
                        1
                    ].includes(data.retcode)
                )
                    this.echo[data.echo].reject(
                        Object.assign(this.echo[data.echo].error, this.echo[data.echo].request, {
                            error: data
                        })
                    )
                else
                    this.echo[data.echo].resolve(
                        data.data
                            ? new Proxy(data, {
        ...
        load() {
            for (const i of [
                ...
                if (!Array.isArray(Bot.wsf[i])) Bot.wsf[i] = []
                Bot.wsf[i].push((ws, ...args) => ws.on("message", data => this.message(data, ws, ...args)))
            }
        }
    })()
)
