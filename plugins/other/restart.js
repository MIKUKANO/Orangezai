...
const temp = {}
const appType = process.env.app_type || process.argv[2]
...
export class Restart extends plugin {
    ...
    init() {
        ...
        if (cfg.bot.stop_cron)
            for (const i of Array.isArray(cfg.bot.stop_cron)
                ? cfg.bot.stop_cron
                : [
                      cfg.bot.stop_cron
                  ])
        ...
        if (cfg.bot.start_cron)
            for (const i of Array.isArray(cfg.bot.start_cron)
                ? cfg.bot.start_cron
                : [
                      cfg.bot.start_cron
                  ])
    ...
    async restartMsg() {
        ...
        const msg = [
            this.getRestartMsg(restart, time)
        ]
    ...
    getRestartMsg(restart, time) {
        if (restart.reason === "dependency") {
            const packages = Array.isArray(restart.workspaces)
                ...
                : Array.isArray(restart.packages)
                  ? restart.packages.join("、")
                  : restart.packages
