...
class PluginsLoader {
    ...
    async deal(e) {
        ...
        for (const i of priority) {
            ...
            if (!lodash.isEmpty(context)) {
                ...
                for (const fnc in context) {
                    ret ||= await Object.assign(new i.class(e), {
                        e
                    })[fnc](context[fnc])
                }
                if (ret === "continue") {
        ...
        for (const i of priority) {
            if (i.plugin.accept) {
                const res = await Object.assign(new i.class(e), {
                    e
                }).accept(e)
                if (res === "return") {
                    return
        ...
        for (const i of priority) {
            if (i.plugin.rule) {
                for (const v of i.plugin.rule) {
                    ...
                    const plugin = Object.assign(new i.class(e), {
                        e
                    })
    ...
    dealEvent(e) {
        ...
        e.logText = `${logger.cyan(e.logText)}${logger.red(
            `[${lodash.truncate(e.msg || e.raw_message || Bot.String(e), {
                length: 100
            })}]`
        )}`
    ...
    async saveCount(e, type) {
        ...
        for (const i of [
            ...
            key.push(`total:${i}`)
            if (e.self_id) {
                key.push(`bot:${e.self_id}:${i}`)
            }
            if (e.user_id) {
                key.push(`user:${e.user_id}:${i}`)
            }
            if (e.group_id) {
                key.push(`group:${e.group_id}:${i}`)
            }
        }
