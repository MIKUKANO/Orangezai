...
export default function setLog() {
    ...
    const defid = chalk.blue(`[OrangeYz]`)
    ...
    chalk.error = (...args) => {
        if (global.Bot) {
            void import("../tools/dependency.js")
                .then(({ installMissingDependency }) => installMissingDependency(args, "logger.error"))
                .catch(err => console.error("Dependency", args, err))
        }
        return chalk.logger.error(defid, ...args)
    }
    global.logger = chalk
}
