...
export default new (class Cfg {
    ...
    watch(file, name, type = "default_config") {
        ...
        this.watcher[key].on("change", path => {
            delete this.config[key]
            clearTimeout(this.changeTimer[key])
            this.changeTimer[key] = setTimeout(() => {
                delete this.changeTimer[key]
    ...
    makeLog(level, msg) {
        if (globalThis.Bot?.makeLog) {
            Bot.makeLog(level, msg, "Config")
            return
        }
        if (globalThis.logger?.logger?.[level]) {
            logger.logger[level](
                logger.blue("[Config]"),
                ...(Array.isArray(msg)
                    ? msg
