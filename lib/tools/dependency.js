...
export async function installMissingDependency(error, source = "") {
    ...
    if (!workspace) {
        notifyOnce(`unresolved:${missing.name}:${source}`, [
    ...
    if (!manifest) {
        notifyOnce(`no-package:${workspace.name}:${missing.name}`, [
    ...
    if (!declared) {
        ...
        notifyOnce(`undeclared:${workspace.name}:${missing.name}`, [
            `${logger.cyan(workspace.name)} 缺少依赖 ${logger.red(missing.name)}`,
            declaredElsewhere
    ...
    if (!isAutoInstallEnabled()) {
        notifyOnce(`disabled:${workspace.name}:${missing.name}`, [
...
async function installPending() {
    ...
    try {
        ...
        Bot.makeLog(
            "mark",
            `插件依赖安装完成 ${logger.green(formatWorkspacePackages(workspaces))}，准备重启`,
            "Dependency"
        )
...
function formatCommand(command) {
    return command.map(item => (/\s/.test(item) ? `"${item}"` : item)).join(" ")
}
