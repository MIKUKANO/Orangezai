...
const serverService = {
    wsConnect(req, socket, head) {
        ...
        if (!this.online && !this.wsf[path]) {
            ...
            const removePending = () => {
                this.ws_pending.delete(pending)
                socket.off("close", removePending)
                socket.off("error", removePending)
            }
            pending.remove = removePending

            this.ws_pending.add(pending)
            socket.once("close", removePending)
            socket.once("error", removePending)
            return
        }
    ...
    flushPendingWs() {
        for (const pending of this.ws_pending) {
            this.ws_pending.delete(pending)
            pending.remove()
            this.wsConnect(pending.req, pending.socket, pending.head)
        }
    },
