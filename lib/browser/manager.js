...
export class BrowserManager {
  ...
  configure(config = {}) {
    ...
    if (config.chromiumPath || cfg?.bot?.chromium_path)
      this.launchOptions.executablePath = config.chromiumPath || cfg?.bot?.chromium_path
    ...
    this.browserTimeout = config.playwrightTimeout || config.puppeteerTimeout || cfg?.bot?.playwright_timeout || cfg?.bot?.puppeteer_timeout || 0
  }
  ...
  async screenshot(name, data = {}, hooks = {}) {
    ...
    try {
      ...
      if (pageGotoParams.waitUntil === "networkidle2")
  ...
  toFileUrl(filePath) {
    return pathToFileURL(path.resolve(cwd, lodash.trim(filePath, "."))).toString()
  }
