...
class PuppeteerCompatPage {
  ...
  async $eval(selector, fn, ...args) {
    return this.page.locator(selector).first().evaluate(fn, ...args)
  }
  ...
  async setContent(html, options = {}) {
    const setContentOptions = { ...options }
  ...
  async setUserAgent(userAgent) {
    return this.page.context().setExtraHTTPHeaders({ "user-agent": userAgent })
  }
...
export async function connect(options = {}) {
  const endpoint = typeof options === "string"
    ? options
    : options.browserWSEndpoint || options.wsEndpoint || options.endpointURL

  if (!endpoint)
    throw new Error("Orangezai puppeteer shim requires browserWSEndpoint/wsEndpoint/endpointURL")

  const browser = options.browserWSEndpoint || options.puppeteerWS || /^https?:/i.test(endpoint)
